...
const css = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  @media(max-width:768px){
    ...
    .sidebar-logo{margin-bottom:0;font-size:16px;} .board-info{display:none;}
    ...
    .main-content{padding:16px;} .content-layout{flex-direction:column;}
    .right-panel-row{flex-direction:column;} .calendar-wrap{width:100%;} .notice-wrap{width:100%;}
    .kanban-board{grid-template-columns:1fr;gap:12px;} .kanban-col{min-height:auto;}
    .stat-grid{gap:8px;} .stat-card{padding:14px 10px;} .stat-num{font-size:24px;} .stat-label{font-size:11px;}
    .page-title{font-size:18px;} .modal-overlay{padding:16px;} .modal{padding:24px 20px;}
  }
  @media(max-width:480px){.main-content{padding:12px;}.auth-card{padding:28px 16px;}.stat-num{font-size:20px;}}
  ...
  return (
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        <div className="field"><label>업무 내용</label>
...
function KanbanView({tasks,setTasks,members,boardId,showToast,currentUser}) {
  ...
  const saveTask=async(task)=>{
    if(task.id){
      ...
      const{data}=await supabase.from("tasks").insert({board_id:boardId,title:task.title,description:task.description||null,assignee:task.assignee,due:task.due||null,status:task.status,created_by:currentUser.name}).select().single();
      setTasks(p=>[...p,data]);
