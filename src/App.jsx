...
const css = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .notice-board{background:#fff;border-radius:var(--radius);padding:20px;box-shadow:var(--shadow);border:1.5px solid var(--border);margin-top:16px;}
...
function NoticeBoard({boardId,currentUser}) {
  ...
  const handleAdd=async()=>{
    ...
    setLoading(true);
    await supabase.from("notices").insert({board_id:boardId,text:input.trim(),author:currentUser.name,done:false});
    setInput("");
    setLoading(false);
  };

  const toggleDone=async(n)=>{
    await supabase.from("notices").update({done:!n.done}).eq("id",n.id);
  };
  ...
  return (
    <div className="notice-board">
      ...
      {notices.length===0&&<div className="no-tasks-msg">공지가 없어요</div>}
      {notices.map(n=>(
